...
static CGFloat kZJDateAndTimePickerViewContentHeight = 330.f;
...
- (void)show:(BOOL)animated completion:(void(^)(void))completion {
    if (animated) {
        [UIView animateWithDuration:0.25 animations:^{
            self.alpha = 1.0;
            self.contentView.transform = CGAffineTransformTranslate(self.contentView.transform, 0, -kZJDateAndTimePickerViewContentHeight);
        }];
    } else {
        self.alpha = 1.0;
        self.contentView.transform = CGAffineTransformTranslate(self.contentView.transform, 0, -kZJDateAndTimePickerViewContentHeight);
...
- (void)setupSubviewsContraints{
    ...
    [_contentView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(kZJDateAndTimePickerViewContentHeight);
    }];
